...
test_that("cache_registry_ddl returns valid SQL", {
  ddl <- CohortDAG:::cache_registry_ddl("main")
...
test_that("ensure_cache_registry creates table in DuckDB", {
  ...
  result2 <- CohortDAG:::ensure_cache_registry(con, "main")
  expect_true(result2)
})
...
test_that("cache_lookup returns FALSE for missing hashes", {
  ...
  result <- CohortDAG:::cache_lookup(con, "main", c("abc123", "def456"))
...
test_that("cache_register and cache_lookup round-trip", {
  ...
  # Register a node
  CohortDAG:::cache_register(con, "main", "hash001", "primary_events",
                                 "main.dagcache_pe_hash001", cohort_ids = c(1L, 2L))

  # Lookup should find it
  result <- CohortDAG:::cache_lookup(con, "main", c("hash001", "hash999"))
...
test_that("cache_register handles duplicate insert (upsert)", {
  ...
  # First insert
  CohortDAG:::cache_register(con, "main", "hashdup", "concept_set",
  ...
  result <- CohortDAG:::cache_lookup(con, "main", "hashdup")
...
test_that("cache_validate returns FALSE when table doesn't exist", {
  ...
  CohortDAG:::cache_register(con, "main", "hashval1", "primary_events",
                                 "main.dagcache_pe_hashval1")

  result <- CohortDAG:::cache_validate(con, "main", "hashval1")
...
test_that("cache_validate returns TRUE when table exists", {
  ...
  # Register it
  CohortDAG:::cache_register(con, "main", "hashval2", "primary_events",
                                 "main.dagcache_pe_hashval2")

  result <- CohortDAG:::cache_validate(con, "main", "hashval2")
...
test_that("cache_touch does not error on empty hashes", {
  ...
  expect_no_error(CohortDAG:::cache_touch(con, "main", character(0)))
})
...
test_that("cache_touch updates last_used_at for existing hashes", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "touchhash", "primary_events",
                                 "main.dagcache_pe_touchhash")

  expect_no_error(CohortDAG:::cache_touch(con, "main", "touchhash"))
})
...
test_that("dag_cache_list returns registered entries", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "listhash1", "concept_set",
                                 "main.dagcache_cs_listhash1", cohort_ids = 1L)
  CohortDAG:::cache_register(con, "main", "listhash2", "primary_events",
...
test_that("dag_cache_stats returns correct stats", {
  ...
  # Empty cache
  CohortDAG:::ensure_cache_registry(con, "main")
  stats <- CohortDAG:::dag_cache_stats(con, "main")
  expect_equal(stats$total_entries, 0L)
  expect_length(stats$by_type, 0)

  # Add entries
  CohortDAG:::cache_register(con, "main", "stathash1", "concept_set",
                                 "main.dagcache_cs_stathash1")
  CohortDAG:::cache_register(con, "main", "stathash2", "concept_set",
                                 "main.dagcache_cs_stathash2")
  CohortDAG:::cache_register(con, "main", "stathash3", "primary_events",
...
test_that("dag_cache_gc handles empty cache", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  expect_message(
    result <- CohortDAG:::dag_cache_gc(con, "main"),
    "Cache is empty"
  )
  expect_equal(nrow(result), 0)
})
...
test_that("dag_cache_gc dry_run reports without deleting", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "gchash1", "primary_events",
  ...
  still_there <- CohortDAG:::dag_cache_list(con, "main")
...
test_that("dag_cache_gc removes orphaned entries", {
  ...
  CohortDAG:::cache_register(con, "main", "orphan1", "primary_events",
                                 "main.dagcache_pe_orphan1")

  # Use very large max_age_days so age doesn't trigger, but orphaned status does
  expect_message(
    result <- CohortDAG:::dag_cache_gc(con, "main", max_age_days = 99999),
    "Removed"
  )
  expect_equal(nrow(result), 1)

  # Cache should now be empty
  after <- CohortDAG:::dag_cache_list(con, "main")
...
test_that("dag_cache_gc with max_age_days = 0 clears all entries", {
  ...
  CohortDAG:::cache_register(con, "main", "gchash2", "primary_events",
...
test_that("dag_cache_clear drops all entries and tables", {
  ...
  CohortDAG:::cache_register(con, "main", "clearhash1", "primary_events",
                                 "main.dagcache_pe_clear1")
  CohortDAG:::cache_register(con, "main", "clearhash2", "concept_set",
                                 "main.dagcache_cs_clear2")

  expect_message(
    CohortDAG:::dag_cache_clear(con, "main"),
...
test_that("cache_register_new_nodes skips concept_set and final_cohort types", {
  ...
  CohortDAG:::cache_register_new_nodes(dag, options, con, "main", cache_misses)

  # Only primary_events should be registered (concept_set and final_cohort are skipped)
  registry <- CohortDAG:::dag_cache_list(con, "main")
