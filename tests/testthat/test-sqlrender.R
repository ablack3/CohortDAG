...
test_that("tokenize_sql handles basic SQL tokens", {
  tokens <- CohortDAG:::tokenize_sql("SELECT a FROM b WHERE c = 1")
...
test_that("tokenize_sql handles quoted strings", {
  tokens <- CohortDAG:::tokenize_sql("SELECT 'hello world' FROM t")
  expect_true(length(tokens) > 0)
...
test_that("tokenize_sql handles parentheses and operators", {
  tokens <- CohortDAG:::tokenize_sql("SELECT (a + b) FROM t WHERE x > 0")
  texts <- vapply(tokens, function(t) t$text, character(1))
...
test_that("ensure_patterns_loaded populates cache without error", {
  ...
  expect_no_error(CohortDAG:::ensure_patterns_loaded(path))
})
...
test_that("escape_dollar_sign escapes $ in strings", {
  result <- CohortDAG:::escape_dollar_sign("price$100")
  expect_true(grepl("\\\\\\$|\\$", result))
})

test_that("str_replace_all replaces multiple occurrences", {
  result <- CohortDAG:::str_replace_all("aabaa", "aa", "x")
...
test_that("render_sql_core substitutes simple parameters", {
  result <- CohortDAG:::render_sql_core(
    "SELECT @a FROM @b",
    c(a = "col1", b = "table1")
  )
...
test_that("split_and_keep returns unchanged when no match", {
  result <- CohortDAG:::split_and_keep("hello world", "[0-9]+")
...
test_that("replace_with_concat handles simple string", {
  result <- CohortDAG:::replace_with_concat("SELECT 'hello'")
...
test_that("evaluate_primitive_condition handles <> comparison", {
  expect_true(CohortDAG:::evaluate_primitive_condition("'x' <> 'y'"))
  expect_false(CohortDAG:::evaluate_primitive_condition("'x' <> 'x'"))
})
...
test_that("evaluate_boolean_condition handles AND", {
  expect_true(CohortDAG:::evaluate_boolean_condition("1 != 0 & 2 != 0"))
  expect_false(CohortDAG:::evaluate_boolean_condition("1 != 0 & 0 != 0"))
})

test_that("evaluate_boolean_condition handles OR", {
  expect_true(CohortDAG:::evaluate_boolean_condition("1 != 0 | 0 != 0"))
  expect_false(CohortDAG:::evaluate_boolean_condition("0 != 0 | 0 != 0"))
})
...
test_that("preceded_by_in detects ' in ' before position", {
  ...
  expect_true(CohortDAG:::preceded_by_in(13, "SELECT x IN (1,2)"))
  expect_false(CohortDAG:::preceded_by_in(8, "SELECT (1,2)"))
})
...
test_that("link_if_then_elses finds condition-then pairs", {
  ...
  ite <- CohortDAG:::link_if_then_elses(str, spans)
...
test_that("render_sql_core handles DEFAULT values", {
  sql <- "{DEFAULT @x = 5}\nSELECT @x"
  result <- CohortDAG:::render_sql_core(sql, character(0))
  expect_true(grepl("5", result))
})

test_that("render_sql_core overrides DEFAULT when param provided", {
  sql <- "{DEFAULT @x = 5}\nSELECT @x"
  result <- CohortDAG:::render_sql_core(sql, c(x = "10"))
...
test_that("extract_defaults parses DEFAULT blocks", {
  ...
  result <- CohortDAG:::extract_defaults(sql)
...
test_that("translate_bigquery lowercases non-quoted tokens", {
  ...
  result <- CohortDAG:::translate_bigquery("SELECT Column1 FROM 'MyTable'")
...
test_that("translate_spark converts CREATE TABLE to SELECT INTO", {
  ...
  skip_if(!file.exists(path))
  CohortDAG:::ensure_patterns_loaded(path)
  result <- CohortDAG:::translate_spark("CREATE TABLE mytbl (a integer, b date)")
...
test_that("render_check_missing_params finds missing parameters", {
  warnings <- CohortDAG:::render_check_missing_params("SELECT @x", c("x", "y"))
...
test_that("token_is_identifier detects identifiers", {
  expect_true(CohortDAG:::token_is_identifier("my_table"))
  expect_true(CohortDAG:::token_is_identifier("column1"))
  expect_false(CohortDAG:::token_is_identifier("my-table"))
...
test_that("translate_sql_with_path errors on unsupported dialect", {
  ...
  CohortDAG:::ensure_patterns_loaded(path)
  expect_error(
    CohortDAG:::translate_sql_with_path("SELECT 1", "nosuchdb", "s1", "", path),
