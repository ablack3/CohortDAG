...
test_that("tokenize_sql tokenizes simple SELECT", {
  tokens <- CohortDAG:::tokenize_sql("SELECT * FROM t")
...
test_that("tokenize_sql handles single-line comment", {
  tokens <- CohortDAG:::tokenize_sql("SELECT 1 -- this is a comment\nSELECT 2")
...
test_that("tokenize_sql handles block comment", {
  tokens <- CohortDAG:::tokenize_sql("SELECT /* block */ 1")
  texts <- vapply(tokens, function(t) t$text, character(1))
  expect_false(any(grepl("block", texts)))
})

test_that("tokenize_sql handles single-quoted string", {
  tokens <- CohortDAG:::tokenize_sql("SELECT 'hello world'")
...
test_that("tokenize_sql handles double-quoted identifier", {
  tokens <- CohortDAG:::tokenize_sql('SELECT "my_col" FROM t')
  in_quotes <- vapply(tokens, function(t) t$inQuotes, logical(1))
  expect_true(any(in_quotes))
})

test_that("tokenize_sql handles --HINT: keyword", {
  tokens <- CohortDAG:::tokenize_sql("--hint SELECT 1")
  texts <- vapply(tokens, function(t) t$text, character(1))
  expect_true("--" %in% texts)
})

test_that("tokenize_sql handles @parameter tokens", {
  tokens <- CohortDAG:::tokenize_sql("SELECT @param FROM t")
  texts <- vapply(tokens, function(t) t$text, character(1))
...
test_that("replace_char_at replaces character at position", {
  result <- CohortDAG:::replace_char_at("hello", 1, "H")
...
test_that("str_replace_region replaces region in string", {
  result <- CohortDAG:::str_replace_region("hello world", 6, 11, "there")
...
test_that("str_replace_all replaces all occurrences", {
  result <- CohortDAG:::str_replace_all("a.b.c", ".", "_")
...
test_that("token_is_identifier returns TRUE for identifiers", {
  expect_true(CohortDAG:::token_is_identifier("hello"))
  expect_true(CohortDAG:::token_is_identifier("table_1"))
  expect_true(CohortDAG:::token_is_identifier("ABC123"))
})
...
test_that("safe_split respects quoted fields", {
  result <- CohortDAG:::safe_split('a,"b,c",d', ",")
...
test_that("split_and_keep splits and retains matches", {
  result <- CohortDAG:::split_and_keep("hello123world", "[0-9]+")
  expect_true(length(result) >= 3)
  expect_true("123" %in% unlist(result))
})

test_that("split_and_keep returns original if no match", {
  result <- CohortDAG:::split_and_keep("hello", "[0-9]+")
...
test_that("find_curly_bracket_spans finds simple spans", {
  spans <- CohortDAG:::find_curly_bracket_spans("{a}{b}")
...
test_that("find_curly_bracket_spans handles empty", {
  spans <- CohortDAG:::find_curly_bracket_spans("no braces")
...
test_that("preceded_by_in returns FALSE at start", {
  expect_false(CohortDAG:::preceded_by_in(1, "hello"))
})

# --- evaluate_primitive_condition ---

test_that("evaluate_primitive_condition handles true/false literals", {
  expect_true(CohortDAG:::evaluate_primitive_condition("true"))
  expect_false(CohortDAG:::evaluate_primitive_condition("false"))
  expect_true(CohortDAG:::evaluate_primitive_condition("1"))
  expect_false(CohortDAG:::evaluate_primitive_condition("0"))
  expect_true(CohortDAG:::evaluate_primitive_condition("!false"))
  expect_false(CohortDAG:::evaluate_primitive_condition("!true"))
...
test_that("evaluate_primitive_condition handles ==", {
  ...
  expect_false(CohortDAG:::evaluate_primitive_condition("'a' == 'b'"))
})
...
test_that("evaluate_primitive_condition handles IN", {
  expect_true(CohortDAG:::evaluate_primitive_condition("'a' IN ('a','b','c')"))
  expect_false(CohortDAG:::evaluate_primitive_condition("'d' IN ('a','b','c')"))
})

test_that("evaluate_primitive_condition handles unrecognized", {
  expect_false(CohortDAG:::evaluate_primitive_condition("rules"))
})

# --- evaluate_boolean_condition ---

test_that("evaluate_boolean_condition handles AND", {
  expect_true(CohortDAG:::evaluate_boolean_condition("true & true"))
  expect_false(CohortDAG:::evaluate_boolean_condition("true & false"))
})

test_that("evaluate_boolean_condition handles OR", {
  expect_true(CohortDAG:::evaluate_boolean_condition("false | true"))
  expect_false(CohortDAG:::evaluate_boolean_condition("false | false"))
})

test_that("evaluate_boolean_condition handles simple", {
  expect_true(CohortDAG:::evaluate_boolean_condition("true"))
  expect_false(CohortDAG:::evaluate_boolean_condition("false"))
})
...
test_that("evaluate_condition handles IN with parentheses", {
  ...
  expect_false(CohortDAG:::evaluate_condition("'c' IN ('a','b')"))
})
...
test_that("extract_defaults handles empty string", {
  defaults <- CohortDAG:::extract_defaults("SELECT 1")
...
test_that("substitute_parameters replaces parameters", {
  ...
  result <- CohortDAG:::substitute_parameters(sql, list(id = "1", name = "'test'"))
...
test_that("substitute_parameters applies defaults", {
  sql <- "{DEFAULT @x = 10}\nSELECT @x"
  result <- CohortDAG:::substitute_parameters(sql, list())
  expect_true(grepl("10", result))
})

test_that("substitute_parameters overrides defaults", {
  sql <- "{DEFAULT @x = 10}\nSELECT @x"
  result <- CohortDAG:::substitute_parameters(sql, list(x = "20"))
...
test_that("substitute_parameters handles longer param names first", {
  ...
  result <- CohortDAG:::substitute_parameters(sql, list(param = "A", param_long = "B"))
...
test_that("render_sql_core renders with parameters and conditionals", {
  ...
  result <- CohortDAG:::render_sql_core(sql, list())
  expect_true(grepl("WHERE x > 0", result))
})

test_that("render_sql_core handles nested conditionals", {
  sql <- "{@a}?{{@b}?{inner}}"
  result <- CohortDAG:::render_sql_core(sql, list(a = "1", b = "1"))
...
test_that("render_check_missing_params finds missing params", {
  ...
  warnings <- CohortDAG:::render_check_missing_params(sql, c("found", "missing"))
...
test_that("render_check_missing_params returns empty for all found", {
  ...
  warnings <- CohortDAG:::render_check_missing_params(sql, c("a", "b"))
...
test_that("parse_search_pattern errors on variable at end", {
  ...
  expect_error(
    CohortDAG:::parse_search_pattern("SELECT @@a FROM @@b"),
    "cannot start or end"
  )
})

test_that("parse_search_pattern parses embedded variable", {
  blocks <- CohortDAG:::parse_search_pattern("SELECT @@a FROM t")
...
test_that("get_key returns value for first match", {
  ...
  expect_equal(CohortDAG:::get_key(obj, c("PascalCase", "camelCase")), 1)
})

test_that("get_key returns second match if first missing", {
  obj <- list(camelCase = 2)
  expect_equal(CohortDAG:::get_key(obj, c("PascalCase", "camelCase")), 2)
})

test_that("get_key returns default if no match", {
  obj <- list(other = 3)
  expect_equal(CohortDAG:::get_key(obj, c("PascalCase", "camelCase"), default = 99), 99)
})

# --- %||% operator ---

test_that("null coalescing returns x when non-null", {
  expect_equal(CohortDAG:::`%||%`(1, 2), 1)
})

test_that("null coalescing returns y when x is null", {
  expect_equal(CohortDAG:::`%||%`(NULL, 2), 2)
})
...
test_that("translate_search finds pattern match", {
  parsed <- CohortDAG:::parse_search_pattern("SELECT @@a FROM")
  m <- CohortDAG:::translate_search("SELECT 1 FROM t", parsed, 1)
  expect_true(m$start > 0)
})

test_that("translate_search returns -1 for no match", {
  parsed <- CohortDAG:::parse_search_pattern("INSERT INTO")
  m <- CohortDAG:::translate_search("SELECT 1 FROM t", parsed, 1)
...
test_that("translate_sql_with_path translates to duckdb", {
  ...
  result <- CohortDAG:::translate_sql_with_path(sql, "duckdb", "abc12345", NULL, NULL)
  expect_true(is.character(result))
...
test_that("translate_sql_with_path errors on unknown dialect", {
  expect_error(
    CohortDAG:::translate_sql_with_path("SELECT 1", "unknown_db_xyz", "abc12345", NULL, NULL),
...
test_that("translate_single_statement_with_path translates single", {
  sql <- "SELECT 1 FROM t"
  result <- CohortDAG:::translate_single_statement_with_path(sql, "duckdb", "abc12345", NULL, NULL)
  expect_true(is.character(result))
})
...
test_that("ensure_patterns_loaded loads replacement patterns", {
  # Force clear cache
  rm(list = ls(CohortDAG:::.translate_pattern_cache), envir = CohortDAG:::.translate_pattern_cache)
...
test_that("translate_sql_apply_patterns applies patterns", {
  CohortDAG:::ensure_patterns_loaded(NULL)
  pats <- get("duckdb", envir = CohortDAG:::.translate_pattern_cache)
  if (length(pats) > 0) {
    sql <- "SELECT DATEADD(day,1,col1) FROM t"
    result <- CohortDAG:::translate_sql_apply_patterns(sql, pats, "abc12345", "")
    expect_true(is.character(result))
  }
})
...
test_that("render_r renders SQL with parameters", {
  result <- CohortDAG:::render_r("SELECT * FROM @schema.person", schema = "main")
  expect_true(grepl("main", result))
})

test_that("render_r handles conditionals", {
  result <- CohortDAG:::render_r("SELECT * FROM t {@include}?{WHERE x > 0}", include = "true")
  expect_true(grepl("WHERE x > 0", result))
})
...
test_that("render_r does not warn when warnOnMissingParameters = FALSE", {
  expect_no_warning(
    CohortDAG:::render_r("SELECT @a, @b FROM t", warnOnMissingParameters = FALSE, a = "1")
  )
})
...
test_that("translate_r translates to duckdb", {
  result <- CohortDAG:::translate_r("SELECT DATEADD(day,1,start_date) FROM t", targetDialect = "duckdb")
  expect_true(is.character(result))
  expect_equal(attr(result, "sqlDialect"), "duckdb")
})

test_that("translate_r translates to postgresql", {
  result <- CohortDAG:::translate_r("SELECT TOP 10 * FROM person", targetDialect = "postgresql")
  expect_true(is.character(result))
