...
test_that("cache_registry_ddl generates DDL", {
  ddl <- CohortDAG:::cache_registry_ddl("results")
...
test_that("cache_table_name builds stable name", {
  ...
  result <- CohortDAG:::cache_table_name(node, "results")
...
test_that("ensure_cache_registry creates registry table", {
  ...
  result <- CohortDAG:::ensure_cache_registry(con, "main")
  expect_true(result)
...
test_that("ensure_cache_registry is idempotent", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  # Call again should not error
  expect_no_error(CohortDAG:::ensure_cache_registry(con, "main"))
})
...
test_that("cache_lookup returns FALSE for empty cache", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  result <- CohortDAG:::cache_lookup(con, "main", c("hash1", "hash2"))
...
test_that("cache_lookup with empty hashes", {
  ...
  result <- CohortDAG:::cache_lookup(con, "main", character(0))
...
test_that("cache_register inserts new entry", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "abc12345", "primary_events", "main.dagcache_pe_abc12345", c(1L, 2L))

  result <- CohortDAG:::cache_lookup(con, "main", "abc12345")
...
test_that("cache_register handles duplicate by updating", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "abc12345", "primary_events", "main.dagcache_pe_abc12345", 1L)
  # Insert same hash again should not error (upsert)
  expect_no_error(CohortDAG:::cache_register(con, "main", "abc12345", "primary_events", "main.dagcache_pe_abc12345", c(1L, 2L)))
})
...
test_that("cache_validate returns FALSE when table doesn't exist", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "hash123", "pe", "main.dagcache_pe_hash123", 1L)
  # Table dagcache_pe_hash123 doesn't actually exist
  result <- CohortDAG:::cache_validate(con, "main", "hash123")
...
test_that("cache_validate with empty hashes", {
  ...
  result <- CohortDAG:::cache_validate(con, "main", character(0))
  expect_equal(length(result), 0)
})
...
test_that("cache_touch updates last_used_at", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "touchme", "pe", "main.dagcache_pe_touchme", 1L)

  # Touch should not error
  expect_no_error(CohortDAG:::cache_touch(con, "main", "touchme"))
})
...
test_that("cache_touch with empty hashes", {
  ...
  expect_invisible(CohortDAG:::cache_touch(con, "main", character(0)))
})
...
test_that("dag_cache_list returns data frame", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "list_hash", "pe", "main.dagcache_pe_list_hash", 1L)

  result <- CohortDAG:::dag_cache_list(con, "main")
...
test_that("cache_register_new_nodes registers cache misses", {
  ...
  options <- list(table_prefix = CohortDAG:::CACHE_TABLE_PREFIX)

  CohortDAG:::cache_register_new_nodes(dag, options, con, "main", c("pe_hash1", "cs_hash1", "fc_hash1"))

  # concept_set and final_cohort should be skipped
  result <- CohortDAG:::cache_lookup(con, "main", c("pe_hash1", "cs_hash1", "fc_hash1"))
...
test_that("emit_cleanup_cached generates cleanup SQL preserving cached tables", {
  ...
  result <- CohortDAG:::emit_cleanup_cached(dag, options, cache_hits = c("pe1", "qe1"), cache_misses = c("ie1", "ce1", "fc1"))
...
test_that("dag_cache_gc with empty cache", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  expect_message(CohortDAG:::dag_cache_gc(con, "main"), "empty")
})
...
test_that("dag_cache_gc dry run", {
  ...
  CohortDAG:::cache_register(con, "main", "orphan1", "pe", "main.dagcache_pe_orphan1", 1L)

  result <- CohortDAG:::dag_cache_gc(con, "main", max_age_days = 0, dry_run = TRUE)
  expect_true(nrow(result) >= 1)
})
...
test_that("dag_cache_gc removes stale entries", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "gc_hash1", "pe", "main.dagcache_pe_gc_hash1", 1L)

  # GC with max_age_days = 0 removes everything
  expect_message(
    result <- CohortDAG:::dag_cache_gc(con, "main", max_age_days = 0),
    "Removed"
  )
  expect_true(nrow(result) >= 1)

  # Verify the entry was removed
  lookup <- CohortDAG:::cache_lookup(con, "main", "gc_hash1")
...
test_that("dag_cache_clear removes all entries", {
  ...
  CohortDAG:::ensure_cache_registry(con, "main")
  CohortDAG:::cache_register(con, "main", "clear1", "pe", "main.dagcache_pe_clear1", 1L)
  CohortDAG:::cache_register(con, "main", "clear2", "qe", "main.dagcache_qe_clear2", 2L)

  expect_message(CohortDAG:::dag_cache_clear(con, "main"), "Cleared 2")
