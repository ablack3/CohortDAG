...
test_that("atlas_json_to_sql generates SQL for single cohort", {
  ...
  result <- CohortDAG:::atlas_json_to_sql(
    json_files[1],
    cohort_id = 1L,
    cdm_schema = "main",
    target_schema = "main",
...
test_that("atlas_json_to_sql_batch non-optimized path", {
  ...
  result <- CohortDAG:::atlas_json_to_sql_batch(
...
test_that("buildBatchCohortQuery builds SQL from cohort list", {
  ...
  cohort_list <- lapply(cohort_set$cohort, function(c) {
    CohortDAG:::cohortExpressionFromJson(as.character(jsonlite::toJSON(c, auto_unbox = TRUE, null = "null")))
  })
  cohort_ids <- as.integer(cohort_set$cohort_definition_id)

  result <- CohortDAG:::buildBatchCohortQuery(
...
test_that("buildBatchCohortQuery errors on mismatched lengths", {
  expect_error(
    CohortDAG:::buildBatchCohortQuery(list(list()), c(1L, 2L)),
    "must match"
  )
})
