...
.translate_stmts_r <- function(stmts, target_dialect) {
  ...
  if (any(analyze_idx)) {
    ...
    # Use gsub with multiline ^ANALYZE to skip comment lines (which start with --)
    # and include COMPUTE STATISTICS in the replacement (not appended at end).
    if (td %in% c("spark", "databricks")) {
      stmts[analyze_idx] <- gsub(
        "(?im)^ANALYZE\\s+(\\S+?)\\s*;?\\s*$",
  ...
  path <- system.file("csv", "replacementPatterns.csv", package = "CohortDAG")
