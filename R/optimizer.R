...
generateCohortSet2 <- function(cdm,
  ...
  for (x in tables_to_check) {
    if (x %in% existingTables) {
      if (overwrite) {
        DBI::dbRemoveTable(con, CDMConnector:::.inSchema(results_schema, x, dbms = db))
      } else {
  ...
  cohortCodelistRef <- if (rlang::is_installed("CDMConnector")) {
    tryCatch(
      CDMConnector:::createAtlasCohortCodelistReference(cdm, cohortSet),
      error = function(e) NULL
    )
  } else {
    NULL
...
generateCohortSetLocal2 <- function(cdm,
  ...
  for (nm in names(cdm)) {
    ...
    tbl <- cdm[[nm]]
    full_name <- CDMConnector:::.inSchema(schema = write_schema, table = nm, dbms = "duckdb")
