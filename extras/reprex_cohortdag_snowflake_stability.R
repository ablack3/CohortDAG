...
# Minimal Snowflake stability repro for CohortDAG::generateCohortSet2().
#
# This mirrors the CDMConnector repro but runs generateCohortSet2() repeatedly
# on the same cohort and same Snowflake CDM to check whether the DAG path is
...
} else if (requireNamespace("CohortDAG", quietly = TRUE)) {
  # use installed package as fallback
} else {
  stop("Need either installed CohortDAG or pkgload.", call. = FALSE)
}
...
read_single_cohort_set <- function(cohort_name, cohort_id, cohort_dir = file.path("inst", "cohorts")) {
  ...
  temp_dir <- tempfile("repro_cohortdag_")
...
table_prefix <- paste0("repronew_", format(Sys.time(), "%H%M%S"), "_", Sys.getpid())
